...
/**
 * Layers are the heart of the engine, as they are base renderer for drawing
 * objects. They are literally a collection of sprites that are drawn within
...
/** @lends pulse.Layer.prototype */
{
  ...
  removeNode : function(name) {
    ...
    if(this.objects.hasOwnProperty(spriteName)) {
      if(this.objects[spriteName] instanceof pulse.Visual) {
        pulse.renderer.removeVisualFromLayer(
          this.objects[spriteName],
          this.name, this.parent.sceneId
        );
      }
  ...
  /**
   * Draws all the visual nodes to the layer's renderer ctx. Then draw's this layer
   */
  draw : function() {
    ...
    if(isDirty) {
      ...
      pulse.renderer.clearLayerSection(
        -this.positionTopLeft.x, -this.positionTopLeft.y,
        engine.size.width, engine.size.height,
        this.name, this.parent.sceneId
      );

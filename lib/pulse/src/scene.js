...
/** @lends pulse.Scene.prototype */
{
  ...
  addLayer : function(layer, zindex) {
       ...
       !this.layers.hasOwnProperty(layer.name))
    {
      ...
      if(this.active === true) {
        ...
        if(appendToEnd === true) {
          //Add a layer on the canvas
            pulse.renderer.addLayer(this.layers[layer.name]);
        } else {
  ...
  draw : function() {
    ...
    for(var o = 0; o < this._private.orderedKeys.length; o++) {
      ...
      if(layer.updated) {
        //get layer context from renderer
        var ctx =  pulse.renderer.getLayerContextByName(layer.name);

        //clear the layer
        pulse.renderer.clearLayer(ctx);

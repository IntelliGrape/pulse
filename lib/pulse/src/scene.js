...
pulse.Scene = pulse.Node.extend(
  /** @lends pulse.Scene.prototype */
  {
    ...
    init: function (params) {
       ...
       * @type {Number}
       */
      this.sceneId = "Scene_" + (~~(Math.random() * 10000000)) + (~~(Math.random() * (Math.random() * 9999999)));
    ...
    addLayer: function (layer, zindex) {
      ...
      if (layer instanceof pulse.Layer && !this.layers.hasOwnProperty(layer.name)) {
        ...
        if (this.active === true) {
          //Add layer to the scene. Args [layer, container, isActive]
    ...
    draw: function () {
      ...
      for (var o = 0; o < this._private.orderedKeys.length; o++) {
        ...
        if (layer.updated) {
          ...
          //clear the layer. Layer name is unique
